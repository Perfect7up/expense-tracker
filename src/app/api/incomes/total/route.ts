...
export async function GET(request: NextRequest) {
  try {
    ...
    // 7. Also get previous period for comparison
    ...
    if (source) {
      previousWhereClause.source = source;
    }

    const previousTotalResult = await prisma.income.aggregate({
    ...
    return NextResponse.json({
      ...
      incomeCount: totalResult._count.id || 0,
    ...
    console.error("GET Income Totals Error:", error);
    return NextResponse.json(
